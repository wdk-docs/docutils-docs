...
:Date: $Date: 2018-06-10 00:22:09 +0200 (So, 10. Jun 2018) $
...
versions 3.3 to 3.5 (cf. `Python聽3 compatibility`_).
...
Python聽3 compatibility
----------------------

The Docutils codebase is written for Python聽2 and uses "on-demand"
translation for `porting to Python聽3`_.

* The `setup.py` script generates Python聽3 compatible sources in
  ``build/`` and tests in ``tests3/`` sub-directories during
  installation_ with Python聽3.
...
* When editing the source, do changes on the Python聽2 versions of the
  files and re-run the build command.

.. _porting to Python聽3: http://docs.python.org/py3k/howto/pyporting.html
...
3. Install the package (you may need root permissions to complete this
   ...
   To install for different Python versions, repeat step聽3 for every

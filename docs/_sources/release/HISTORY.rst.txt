...
:Date: $Date: 2018-07-12 12:17:15 +0200 (Do, 12. Jul 2018) $
:Revision: $Revision: 8224 $
...
* setup.py
  ...
  - Fix [ 3527842 ]. Under Python聽3, converted tests and tools were
    ...
    ``setup.py install`` under Python聽3, remove the spurious
...
* docutils/io.py
  ...
  - Fix writing binary data to sys.stdout under Python聽3 (allows
...
* General:
  ...
  - Decode command line arguments with the locale's preferred encoding
    (to allow, e.g., ``--title=Dornr枚schen``).
...
* docutils/io.py:
  ...
  - Do not close() sys.stdin, sys.stdout, or sys.stderr. Prevents
    ``Exception ValueError: 'I/O operation on closed file.'`` with Python聽3.
...
* setup.py:
  ...
  - Python聽3 support: copy test/ and tools/ to the build-dir
...
* docutils/parsers/rst/states.py:
  ...
  - Added support for Unicode inline markup delimiters "鈥� 鈥� 鈥� 鈥� 鈥�" and
    "聽" (non-breaking space), and "隆 驴" openers.
...
* docutils/parsers/rst/states.py:
  ...
  - Added support for multiple attributions in a physical block quote
    ...
    quotes.
  - Added support for unicode bullets in bullet lists: "鈥�", "鈥�", and
    "鈦�".
